		...
		if ( $holder ) {
			...
			if ( $is_self_hosted ) {
				...
				if ( $is_youtube ) {
					...
					printf( '<iframe class="thb-video-api thb-video-youtube" id="%s" type="text/html" src="//www.youtube.com/embed/%s?enablejsapi=1&modestbranding&showinfo=0&rel=0%s" frameborder="0"></iframe>', $id, $code, $params );
				}
				elseif( $is_vimeo ) {
					if ( $loop ) {
						$params .= '&loop=1';
...
if( !function_exists('thb_page_title') ) {
	...
	function thb_page_title( $text, $class = 'page-title', $id = '' ) {
		...
		printf( "<%s class='%s' id='%s'>%s</%s>", $title_tag, $class, $id, $text, $title_tag );
	}
}