...
function thb_woocommerce_skin() {
	if( thb_woocommerce_check() && thb_woocommerce_config('skin') ) {
...
function thb_woocommerce_options() {
	if( thb_woocommerce_check() && thb_woocommerce_config('options') ) {
		$thb_theme = thb_theme();
		$thb_page = $thb_theme->getAdmin()->getMainPage();
			...
			$thb_tab = new THB_Tab( __('WooCommerce', 'thb_text_domain'), 'woocommerce' );
				...
				if ( thb_woocommerce_config('sidebar_shop') ) {
				...
				if ( thb_woocommerce_config('sidebar_product') ) {
...
function thb_shop_columns_layout( $classes ) {
	...
	if( is_woocommerce() && ( is_shop() || is_tax() ) ) {
		$classes[] = 'thb-shop-' . thb_loop_columns() .'col';
	}
...
function thb_shop_sidebar( $classes ) {
	...
	if( is_woocommerce() ) {
		...
		$classes[] = thb_get_sidebar_position();
	}
 ...
 * @return string
 */
function thb_get_sidebar_position() {
	if ( !thb_woocommerce_check() ) {
		return;
	}
